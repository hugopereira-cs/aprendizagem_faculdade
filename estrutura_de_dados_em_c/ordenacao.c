...
// --- DECLARAÇÃO DAS FUNÇÕES (PROTÉTIPOS) ---
...
int main() {
  ...
  printf("--- GERANDO LISTA DE ENTRADA ---\n");
  ...
  // dos dados antes de cada ordenação. Isso é crucial para garantir que cada algoritmo receba a mesma lista de entrada original.
  ...
  bubbleSort(listaTemp, TAMANHO);
  imprimirLista("Bubble Sort | Caso Médio (aleatória)", listaTemp, TAMANHO);
  printf("\n");
  
  // 2. TESTANDO O INNSERTION SORT
  printf("========== 2. INSERTION SORT ==========\n");
  
  // a) Melhor Caso: Lista já ordenada
  copiarLista(listaTemp, listaOrdenada, TAMANHO);
  insertionSort(listaTemp, TAMANHO);
  imprimirLista("Insertion Sort | Pior Caso (já ordenada)", listaTemp, TAMANHO);
  
  // b) Pior Caso: Lista em ordem inversa
  copiarLista(listaTemp, listaInversa, TAMANHO);
  insertionSort(listaTemp, TAMANHO);
  imprimirLista("Insertion Sort | Melhor Caso (ordem inversa)", listaTemp, TAMANHO);
 ...
 *          na ordem errada. Repete esse processo até que a lista esteja orenada.
...
void bubbleSort(int lista[], int n) {
  ...
  for (i = 0; i < n -1; i++) {
    ...
    // Laço interno percorre a lista, comparandoelementos adjacentes.
    // O limite é 'n -1 -1' porque os elementos no final já estão ordenados.
    for (j = 0; j < n - i - 1; j++) {
    ...
    // a lista já esta'ordenada e podemos parar o algoritmo.
    if (trocou == 0) {
      break;
    }
  }
...
void insertionSort(int lista[], int n) {
  ...
  for (i = 1; i < n; i++) {
    ...
    // uma posiçaõ para a direita, para abrir espaço para a inserção.
 ...
 * @details Lógica: Divide a lista em duas partes, ordenada (à esquerda) e não ordenada à (direita). A cada passo,
 *          encontra o menor elemento na parte não ordenada e o troca com o primeiro elemento da parte não ordenada.
 * 
...
void selectionSort(int lista[], int n){
  ...
  for (i = 0; i < n - 1; i++) {
    ...
    // Laço interno percorre a parte não ordenada pra encontrar o menor elemento.
    for (j = i + 1; j < n; j++) {
...
/**
 * @brief Gera uma lista de 'n' elementos com valores aleatórios.
 */
void gerarListaAleatoria(int lista[], int n) {
  for (int i = 0; i < n; i++) {
...
/**
 * @brief Imprime uma lista de 'n' elementos com um título descritivo.
 */
void imprimirLista(const char* titulo, const int lista[], int n) {
  printf("%s: ", titulo);
  for (int i = 0; i < n; i++) {
